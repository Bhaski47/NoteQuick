...
function ToDo() {
  ...
  useEffect(() => {
    const getDetails = async () => {
      try {
        const response = await axios.post(`${process.env.REACT_BACKEND_URL}/get`, {
          email: email,
        });
        if (email.trim() === "") {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios.put(`${process.env.REACT_BACKEND_URL}/update/${item._id}`, {
      title: title,
      content: content,
    });
  ...
  const addNote = async (e) => {
    ...
    const fetchInfo = await axios.post(`${process.env.REACT_BACKEND_URL}/insert`, {
      email: email,
      title: title,
      content: content,
    });
  ...
  const handleDelete = async (item) => {
    try {
      await axios.delete(`${process.env.REACT_BACKEND_URL}/delete/${item._id}`, {
        email: email,
      });
      setFetch((prev) => !prev);
    } catch (error) {
